...
function renderOrgChart(){
  ...
  function buildNode(emp){
    ...
    let html = `<li><div class="org-card" onclick="openEmployeDetail('${emp.id}')">
        <div class="avatar">${initials}</div>
        <div class="name">${emp.prenom} ${emp.nom}</div>
        <div class="role">${poste.intitule}</div>
      </div>`;
    if(children.length>0){
      html += "<ul>" + children.map(buildNode).join("") + "</ul>";
    }
    html += "</li>";
    return html;
  }

  container.innerHTML = `<div class="org-tree"><ul>${roots.map(buildNode).join("")}</ul></div>`;
}
